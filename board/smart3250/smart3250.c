 ...
 */
static void smart3250_ssp0_init(void)
{
...
static int smart3250_sspxfer(u_char *out,
                               u_char *in,
                               int bytes)
{
  int rbytes = 0, xfrd = 0;
...
static u_char smart3250_sspread(int index)
{	
	...
	datao [3] = 0xFF;
	smart3250_sspxfer(datao, datai, 4);
	byte = datai [3];

	return byte;
}

void smart3250_get_board_info(void)
{
	u_char *p8;
	int idx;
	...
	{
		/* Set some defaults */
		phyhwdesc.dramcfg = (PHYHW_DRAM_TYPE_SDRAM | PHYHW_DRAM_SIZE_64M);
